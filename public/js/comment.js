...
function renderComment(){
    ...
    if(page<=1){
        page=1;
        previous.html('<span><b aria-hidden="true">&larr;</b> 没有上一页了</span>');
    }else{
        previous.html('<li class="previous"><a href="javascript:;"><span aria-hidden="true">&larr;</span>上一页</a></li>');
    }
    if(page>=pages){
        page=pages;
        next.html('<span><b aria-hidden="true">&larr;</b> 没有下一页了</span>');
    }else{
        next.html('<li class="next"><a href="javascript:;"><span aria-hidden="true">&larr;</span>下一页</a></li>');
    }
    ...
    if(comments.length==0){
        ...
        for(var i=start;i<end;i++){
            html+='<div class="panel-body"><div class="row"><div class="col-xs-6">'+comments[i].username+
                    '</div><div class="col-xs-6" style="text-align: right">'+formatDate(comments[i].postTime)+'</div></div>'+
                '<div class="row"><div class="col-xs-12">'+comments[i].content+'</div></div></div>';
        }
